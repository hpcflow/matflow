...
def write_model_py_script(path, material_parameters, numerics):
    ...
    TEMPLATE = dedent(
        ...
        def rotate_tensor_rank_4(R, T):
            ...
            return np.sum(
                R0 * R1 * R2 * R3 * T[None, :, None, :, None, :, None, :], axis=(1, 3, 5, 7)
            )
        ...
        class CrystalPlasticity(Problem):
            def custom_init(self, quat, cell_ori_inds):
                r = {r}
                self.gss_initial = {gss_initial}
                
                input_slip_sys = onp.loadtxt("input_slip_sys.txt")
                num_slip_sys = len(input_slip_sys)

                slip_directions = input_slip_sys[:, self.dim :]
                slip_directions = (
                    slip_directions / onp.linalg.norm(slip_directions, axis=1)[:, None]
                )
                ...
                for i in range(num_slip_sys):
                    for j in range(num_directions_per_normal):
                        self.q[
                            i, i // num_directions_per_normal * num_directions_per_normal + j
                        ] = 1.0
                ...
                Fp_inv_gp = onp.repeat(
                    onp.repeat(
                        onp.eye(self.dim)[None, None, :, :], len(self.fes[0].cells), axis=0
                    ),
                    self.fes[0].num_quads,
                    axis=1,
                )
                ## Hu: slip resistance
                slip_resistance_gp = self.gss_initial * onp.ones(
                    (len(self.fes[0].cells), self.fes[0].num_quads, num_slip_sys)
                )
                ## Hu: slip rate
            ...
            def get_maps(self):
                ...
                def get_partial_tensor_map(Fp_inv_old, slip_resistance_old, slip_old, rot_mat):
                    ...
                    def first_PK_stress(u_grad):
                        x, _ = jax.flatten_util.ravel_pytree(
                            [u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat]
                        )
                        y = newton_solver(x)
                        S = unflatten_fn(y)
                        _, _, _, Fe, F = helper(
                            u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, S
                        )
                        sigma = 1.0 / np.linalg.det(Fe) * Fe @ S @ Fe.T
                    ...
                    def update_int_vars(u_grad):
                        ...
                        y = newton_solver(x)
                        S = unflatten_fn(y)
                        Fp_inv_new, slip_resistance_new, slip_new, Fe, F = helper(
                            u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, S
                        )
                    ...
                    def helper(u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, S):
                        tau = np.sum(
                            S[None, :, :]
                            * rotate_tensor_rank_2_vmap(rot_mat, self.Schmid_tensors),
                            axis=(1, 2),
                        )
                        gamma_inc = (
                            ao
                            * self.dt
                            * np.absolute(tau / slip_resistance_old) ** (1.0 / xm)
                            * np.sign(tau)
                        )

                        tmp = (
                            h
                            * np.absolute(gamma_inc)
                            * np.absolute(1 - slip_resistance_old / t_sat) ** gss_a
                            * np.sign(1 - slip_resistance_old / t_sat)
                        )
                    ...
                    def implicit_residual(x, y):
                        ...
                        S_ = np.sum(
                            rotate_tensor_rank_4(rot_mat, self.C)
                            * 1.0
                            / 2.0
                            * (Fe.T @ Fe - np.eye(self.dim))[None, None, :, :],
                            axis=(2, 3),
                        )
                    ...
                    def newton_solver(x):
                        ...
                        def body_fun(state):
                            ...
                            relax_param_ini = 1.0
                            sub_step_ini = 0
                            max_sub_step = {newton_max_sub_step}

                            def sub_cond_fun(state):
                                _, crt_res_vec, sub_step = state
                                return np.logical_and(
                                    np.linalg.norm(crt_res_vec) >= np.linalg.norm(res_vec),
                                    sub_step < max_sub_step,
                                )
                            ...
                            relax_param_f, res_vec_f, _ = jax.lax.while_loop(
                                sub_cond_fun,
                                sub_body_fun,
                                (relax_param_ini, res_vec, sub_step_ini),
                            )
                            step_update = step + 1

                            return step_update, res_vec_f, y + 2.0 * relax_param_f * y_inc

                        step_f, res_vec_f, y_f = jax.lax.while_loop(
                            cond_fun, body_fun, (step, res_vec, y0)
                        )
                ...
                def tensor_map(u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat):
                    first_PK_stress, _ = get_partial_tensor_map(
                        Fp_inv_old, slip_resistance_old, slip_old, rot_mat
                    )
                    return first_PK_stress(u_grad)

                def update_int_vars_map(
                    u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat
                ):
                    _, update_int_vars = get_partial_tensor_map(
                        Fp_inv_old, slip_resistance_old, slip_old, rot_mat
                    )
            ...
            def update_int_vars_gp(self, sol, params):
                ...
                Fp_inv_gp, slip_resistance_gp, slip_gp, rot_mats_gp = vmap_update_int_vars_map(
                    u_grads, *params
                )
                # TODO
            ...
            def inspect_interval_vars(self, params):
                ...
                print(
                    f"slip_resistance index 0 = {{slip_resistance_0}}, max slip_resistance = {{np.max(slip_resistance_gp)}}"
                )
            ...
            def compute_avg_stress(self, sol, params):
                ...
                def P_to_sigma(P, F):
                    return 1.0 / np.linalg.det(F) * P @ F.T
                ...
                sigma_cell_data = (
                    np.sum(sigma * self.fes[0].JxW[:, :, None, None], 1)
                    / np.sum(self.fes[0].JxW, axis=1)[:, None, None]
                )

                # num_cells*num_quads, vec, dim) * (num_cells*num_quads, 1, 1)
                avg_P = np.sum(
                    P.reshape(-1, self.fes[0].vec, self.dim)
                    * self.fes[0].JxW.reshape(-1)[:, None, None],
                    0,
                ) / np.sum(self.fes[0].JxW)
    ...
    with path.open("wt") as fh:
        fh.write(
            TEMPLATE.format(
                C11=material_parameters["C11"],
                C12=material_parameters["C12"],
                C44=material_parameters["C44"],
                r=material_parameters["r"],
                gss_initial=material_parameters["gss_initial"],
                h=material_parameters["h"],
                t_sat=material_parameters["t_sat"],
                gss_a=material_parameters["gss_a"],
                ao=material_parameters["ao"],
                xm_d=material_parameters["xm_d"],
                newton_max_sub_step=numerics["newton_max_sub_step"],
            )
        )
