...
class LoadStep(ParameterValue):
    ...
    def random_inc(
        ...
        num_increments: int,
        target_def_grad: float,
        start_def_grad: ArrayLike | None = None,
        dump_frequency: int = 1,
    ) -> LoadStep:
        ...
        dump_frequency : int, optional
            By default, 1, meaning results are written out every increment.
        """
        _method_name = "random_inc"
        _method_args = {
            "total_time": total_time,
            "num_increments": num_increments,
            "target_def_grad": target_def_grad,
            "start_def_grad": start_def_grad,
            "dump_frequency": dump_frequency,
        }
...
class LoadCase(ParameterValue):
    ...
    def multistep_random_inc(
        ...
        interpolate_kind: str | int = 3,
    ) -> LoadCase:
        """A load case with multiple `LoadStep.random_inc` which are interpoled 
        over a number over a steps to produce smooth transition between load cases.

        Parameters
        ----------
        steps
            A list of `dict`s representing `LoadStep.random_inc` object 
            constructions. with added key `repeats`.
        interpolate_steps:
            Number of step to interpolate each loadcase over
        interpolate_kind:
            Kind of interpolation, passed to `scipy.interpolate.interp1d`
